...
export const EventBidsList: React.FC = () => {
      ...
      <div className="max-w-4xl mx-auto py-12">
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-8 text-center">
          ...
          <p className="text-[#16232A]/70 mb-2">
            All vendor bidding and negotiation activities are handled by your
            Event Planner.
          </p>
          <p className="text-sm text-[#16232A]/60 mb-6">
            Your planner will finalize vendors and keep you updated throughout
            the process.
          </p>
  ...
  // Filter bids
  const filteredGroupedBids = Object.entries(groupedBids).reduce(
    (acc, [service, serviceBids]) => {
      if (selectedService === "all" || service === selectedService) {
        const filtered = serviceBids.filter((bid) => {
          ...
          const matchesPrice =
            bid.offeredPrice >= minPrice && bid.offeredPrice <= maxPrice;
  ...
  const fetchBids = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}/bids/event/${eventId}`,
        {
      ...
      const mapped = data.map((b: any) => ({
        ...
        vendorRating: b.vendor?.rating || 4,
        ...
        status: b.status, // 🔥 IMPORTANT
        negotiationCount: b.negotiation_count || 0,
      }));
