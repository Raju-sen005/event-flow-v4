...
export const VendorKYC: React.FC<VendorKYCProps> = ({
  ...
  const [isEditing, setIsEditing] = useState(kycData.status === 'not_submitted' || kycData.status === 'rejected');

  const isReadOnly = kycData.status === 'under_review' || kycData.status === 'verified';
  ...
  const handleSubmit = () => {
    // Validation
    if (!kycData.identityProofType || !kycData.identityFront || !kycData.identityBack) {
      alert('Please upload both front and back of your identity proof');
      return;
    }
    if (!kycData.businessProofType || !kycData.businessProof) {
      alert('Please upload your business proof');
      return;
    }
    if (!kycData.accountHolderName || !kycData.bankName || !kycData.accountNumber || !kycData.ifscCode || !kycData.bankProof) {
      alert('Please complete all bank details and upload bank proof');
      return;
    }
    if (!kycData.addressProofType || !kycData.addressProof) {
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-xl font-bold text-[#16232A]">KYC Verification Status</h2>
            </div>
            <p className="text-gray-600 text-sm">Complete your KYC to unlock bidding and withdrawal features</p>
          </div>
          {getStatusBadge()}
        </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-red-800 mb-2">
                  <strong>Reason:</strong> {kycData.rejectionReason || 'Document verification failed'}
                </p>
          ...
          <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            {kycData.submittedAt && (
              <p className="text-xs text-blue-600 mt-2">
                Submitted on {new Date(kycData.submittedAt).toLocaleDateString('en-IN', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
                  hour: '2-digit',
          ...
          <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-green-800 font-semibold">
                Your KYC is verified! You can now bid on events and manage withdrawals.
              </p>
            </div>
            {kycData.verifiedAt && (
              <p className="text-xs text-green-600 mt-2">
                Verified on {new Date(kycData.verifiedAt).toLocaleDateString('en-IN', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric'
                })}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Upload Aadhaar or PAN card (Front & Back)</p>
            </div>
          </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                ...
                {kycData.identityFront ? (
                  <div className="p-4 border border-green-200 bg-green-50 rounded-lg">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <CheckCircle className="h-5 w-5 text-green-600" />
                        <div>
                          <p className="text-sm font-semibold text-gray-900">{kycData.identityFront.fileName}</p>
                          <p className="text-xs text-gray-600">{kycData.identityFront.fileSize}</p>
                        </div>
                      </div>
                      {!isReadOnly && (
                  ...
                  <label className={`block border-2 border-dashed border-gray-300 rounded-lg p-6 text-center ${!isReadOnly ? 'cursor-pointer hover:border-[#FF5B04] transition-colors' : 'cursor-not-allowed opacity-50'}`}>
                    <input
                      type="file"
                      accept="image/*,.pdf"
                      onChange={(e) => handleFileUpload('identityFront', e)}
                  ...
                  <div className="p-4 border border-green-200 bg-green-50 rounded-lg">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <CheckCircle className="h-5 w-5 text-green-600" />
                        <div>
                          <p className="text-sm font-semibold text-gray-900">{kycData.identityBack.fileName}</p>
                          <p className="text-xs text-gray-600">{kycData.identityBack.fileSize}</p>
                        </div>
                      </div>
                      {!isReadOnly && (
                        <label className="cursor-pointer">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Upload GST Certificate, Shop Act, or Trade License</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => handleInputChange('businessProofType', e.target.value)}
                disabled={isReadOnly}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FF5B04]/20 focus:border-[#FF5B04] disabled:bg-gray-100 disabled:cursor-not-allowed"
              >
                <option value="">Select document type</option>
              ...
              <div className="p-4 border border-green-200 bg-green-50 rounded-lg">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <CheckCircle className="h-5 w-5 text-green-600" />
                    <div>
                      <p className="text-sm font-semibold text-gray-900">{kycData.businessProof.fileName}</p>
                      <p className="text-xs text-gray-600">{kycData.businessProof.fileSize}</p>
                    </div>
                  </div>
                  {!isReadOnly && (
                    <label className="cursor-pointer">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Provide account details for payments</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Cancelled Cheque / Bank Proof</label>
              {kycData.bankProof ? (
                <div className="p-4 border border-green-200 bg-green-50 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <CheckCircle className="h-5 w-5 text-green-600" />
                      <div>
                        <p className="text-sm font-semibold text-gray-900">{kycData.bankProof.fileName}</p>
                        <p className="text-xs text-gray-600">{kycData.bankProof.fileSize}</p>
                      </div>
                    </div>
                    {!isReadOnly && (
                      <label className="cursor-pointer">
                ...
                <label className={`block border-2 border-dashed border-gray-300 rounded-lg p-6 text-center ${!isReadOnly ? 'cursor-pointer hover:border-[#FF5B04] transition-colors' : 'cursor-not-allowed opacity-50'}`}>
                  ...
                  />
                  <Upload className="h-8 w-8 text-gray-400 mx-auto mb-2" />
                  <p className="text-sm text-gray-600">Upload cancelled cheque or bank statement</p>
                  <p className="text-xs text-gray-500 mt-1">Image or PDF, max 5MB</p>
                </label>
              )}
            </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Upload utility bill, rental agreement, or official document</p>
            </div>
          </div>
              ...
              <div className="p-4 border border-green-200 bg-green-50 rounded-lg">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <CheckCircle className="h-5 w-5 text-green-600" />
                    <div>
                      <p className="text-sm font-semibold text-gray-900">{kycData.addressProof.fileName}</p>
                      <p className="text-xs text-gray-600">{kycData.addressProof.fileSize}</p>
                    </div>
                  </div>
                  {!isReadOnly && (
                    <label className="cursor-pointer">
